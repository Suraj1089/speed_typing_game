...
// ]at unclear, a friend is a fridge from the right perspective. An upset carriage is a stitch of the mind. To be more specific, a temper is a pair from the right perspective. Authors often misinterpret the liquid as a notchy baseball, when in actuality it feels more like an unbarbed angle. Though we assume the latter, the first vagrom report is, in its own way, a tower. We know that the octopus of a cd becomes an unrent dahlia.",
//     "A reptant discussion's rest comes with it the thought that the condemned syrup is a wish. The drake of a wallaby becomes a sonant harp. If this was somewhat unclear, spotty children show us how technicians can be jumps. Their honey was, in this moment, an intime direction. A ship is the lion of a hate. They were lost without the croupous jeep that composed their lily. In modern times a butcher of the birth is assumed to be a spiral bean.",
//     "Those cowbells are nothing more than elements. This could be, or perhaps before stockings, thoughts were only opinions. A coil of the exclamation is assumed to be a hurtless toy. A board is the cast of a religion. In ancient times the first stinko sailboat is, in its own way, an exchange. Few can name a tutti channel that isn't a footless operation. Extending this logic, an oatmeal is the rooster of a shake. Those step-sons are nothing more than matches."
// ];
var paragraphs = "suraj pisal";
...
function initTimer() {
    if(timeLeft > 0) {
        ...
        wpmTag.innerText = wpm;
           ...
           Your result is: ${wpmTag.innerText} wpm
...
$(document).ready(function() {
    ...
    const serverURL = `http://localhost:8000/loadparagraph?time=30&difficulty=Hard`;
  
    // Check if the paragraph is already present in localStorage
    const storedParagraph = localStorage.getItem('paragraph');
    if (storedParagraph) {
      // If it exists in localStorage, use it directly
      console.log('Paragraph found in localStorage.');
      displayParagraph(storedParagraph);
    } else {
      // If not, make an AJAX request to fetch the paragraph from the server
      $.ajax({
        url: serverURL,
        method: "GET",
        dataType: "text",
        success: function(data) {
          // Once the paragraph is fetched successfully, store it in localStorage
          localStorage.setItem('paragraph', data);
          displayParagraph(data);
        },
        error: function(err) {
          // Handle any errors that occur during the AJAX request
          alert("Failed to load the paragraph." + String(err.data));
        }
      });
    }
  
    // Function to display the paragraph in the paragraph element
    function displayParagraph(paragraph) {
      console.log(paragraph);
      // Your logic to display the paragraph in the paragraph element goes here
    }
  });
